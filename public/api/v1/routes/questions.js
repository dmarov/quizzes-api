...
routes.get(path, user(), quiz(),
    async (ctx, next) => {
        ...
        let _links = await hal.questions.links({ origin, userName, quizId, offset, limit, total });
        ...
        try {
            ...
            questions = await ctx.db.withTransaction(async tx => {
                ...
                for(let patch of diff) {
                    ...
                    if (patch.quiz_id || patch.content || patch.response || patch.creation_date || patch.tags)
